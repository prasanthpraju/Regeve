  ...
  const CandidateDashboard = ({ token = null }) => {
    ...
    // Fetch candidates from backend
    // Fetch candidates from backend - FIXED
const fetchCandidates = async () => {
  try {
    const response = await axiosInstance.get("/candidates", {
      params: { 
        populate: ["photo", "election_candidate_position"],
        "filters[election][id][$eq]": electionData.electionId
      },
    });

    // Debug: check API response
    console.log("API Candidates Response:", response.data.data);

    const candidatesData = response.data.data.map((candidate) => {
      // Get section ID correctly
      const sectionId = candidate.election_candidate_position?.id || 
                       candidate.section;
      ...
      return {
        ...
        section: sectionId,
        position: candidate.election_candidate_position?.Position || 
                 candidate.position || 
                 "No Position",
    ...
    // Update sections with filtered candidates
    const updatedSections = sections.map((section) => {
      const sectionCandidates = candidatesData.filter((c) => {
        // Compare both as numbers
        return Number(c.section) === Number(section.id);
      });
                  ...
                  <div className="flex flex-col sm:grid sm:grid-cols-2 gap-3 sm:gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className="text-slate-500 text-xs font-medium uppercase tracking-wide">PHONE</span>
                      </div>
                    ...
                    <div className="mt-3 sm:mt-0">
                      <div className="flex items-center gap-2 mb-1">
                        <div className="w-6 h-6 sm:w-8 sm:h-8 bg-emerald-100 rounded flex items-center justify-center flex-shrink-0">
                          <span className="text-emerald-600 text-xs font-bold">W</span>
                        </div>
                        <span className="text-slate-500 text-xs font-medium uppercase tracking-wide">WHATSAPP</span>
                      </div>
                      <p className="text-slate-900 font-medium text-sm sm:text-base mt-1 sm:mt-0 ml-8 sm:ml-0">
                        {selectedCandidate.whatsapp}
                      </p>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4">
                  ...
                  <div>
                    <div className="mb-1 sm:mb-2">
                      <span className="text-slate-500 text-xs sm:text-sm font-medium uppercase tracking-wide">AGE</span>
                    </div>
                  ...
                  <div>
                    <div className="mb-1 sm:mb-2">
                      <span className="text-slate-500 text-xs sm:text-sm font-medium uppercase tracking-wide">GENDER</span>
                    </div>
                    <div className="flex items-center">
                  ...
                  <div className="col-span-2 sm:col-span-2">
                    <div className="mb-1 sm:mb-2">
                      <span className="text-slate-500 text-xs sm:text-sm font-medium uppercase tracking-wide">APPLIED DATE</span>
                    </div>
                    <div className="flex items-center">
                      <span className="text-slate-900 font-bold text-lg sm:text-xl">
